/*
	*By LowFlyZone*
	...
	nul = [this] execVM "sendUnitLoadoutCodeToServer.sqf";
	nul = [player] execVM "sendUnitLoadoutCodeToServer.sqf";
	nul = [unit1] execVM "sendUnitLoadoutCodeToServer.sqf";
...
_nameVarUnit=vehicleVarName _unit;
_gearText = _gearText + "removeAllWeapons  "+_nameVarUnit;
_gearText = _gearText + "removeAllAssignedItems  "+_nameVarUnit;
_gearText = _gearText + "removeAllContainers  "+_nameVarUnit;
_gearText = _gearText + "removeHeadgear  "+_nameVarUnit;
_gearText = _gearText + "removeGoggles  "+_nameVarUnit;

if ((headgear _unit)!="") then {
    _headGear = headgear _unit;
    _gearText = _gearText + format [_nameVarUnit+ " addHeadgear '%1'; ", _headGear];
};
if ((goggles _unit)!="") then {
    _goggles = goggles _unit;
    _gearText = _gearText + format [_nameVarUnit+ " addGoggles '%1'; ", _goggles];
};
if ((uniform _unit)!="") then {
    _uniform = uniform _unit;
    _gearText = _gearText + format [_nameVarUnit+ " addUniform '%1'; ", _uniform];
};
if ((vest _unit)!="") then {
    _vest = vest _unit;
    _gearText = _gearText + format [_nameVarUnit+ " addVest '%1'; ", _vest];
};
if ((backpack _unit)!="") then {
    _backpack = backpack _unit;
    _gearText = _gearText + format [_nameVarUnit+ " addBackPack '%1'; ", _backpack];
};
...
_blacklist = [""];
{
    if ((_x select 0)!="" && !((_x select 0) in _blacklist)) then {
        ...
        if (count(_x)>4) then {
            //_gearText = _gearText + format [_nameVarUnit+ " addmagazine '%1'; ", [(_x select 4) select 0,(_x select 4) select 1]]; //weapon starts with no magazine!
            _gearText = _gearText + format [_nameVarUnit+ " addmagazine '%1'; ", ((_x select 4) select 0)];
        };
        /*if(count(_x)>5) then {
        	//_gearText = _gearText + format [_nameVarUnit+ " addmagazine '%1'; ", [(_x select 5) select 0,(_x select 5) select 1]]; //weapon starts with no magazine!
        	_gearText = _gearText + format [_nameVarUnit+ " addmagazine '%1'; ", ((_x select 5) select 0)];
        };*/
        if ((_x select 0) != "")then{_gearText = _gearText + format [_nameVarUnit+ " addweapon '%1'; ", (_x select 0)];
                                        };
        //if((_x select 1) != "")then{_gearText = _gearText + format [_nameVarUnit+ " linkItem '%1'; ", (_x select 1)];}; //does not add the attachment
        //if((_x select 2) != "")then{_gearText = _gearText + format [_nameVarUnit+ " linkItem '%1'; ", (_x select 2)];}; //does not add the attachment
        //if((_x select 3) != "")then{_gearText = _gearText + format [_nameVarUnit+ " linkItem '%1'; ", (_x select 3)];}; //does not add the attachment
    };
}
foreach (weaponsItems _unit);
//Now the attachments get added!
{
    if (_x != "") then {_gearText = _gearText + format [_nameVarUnit+ " addPrimaryWeaponItem '%1'; ", _x];
                           };
}
foreach (primaryWeaponItems _unit);

{
    if (_x != "") then {_gearText = _gearText + format [_nameVarUnit+ " addHandgunItem '%1'; ", _x];
                           };
}
...
if (count(_array)>0) then {
    {
        for [{_i=0},{_i<((_array) select 1) select _forEachIndex},{_i=_i+1}] do {
                _gearText = _gearText + format [_nameVarUnit+ " addItemToUniform '%1'; ", _x];
            };
...
if (count(_array)>0) then {
    {
        for [{_i=0},{_i<((_array) select 1) select _forEachIndex},{_i=_i+1}] do {
                _gearText = _gearText + format [_nameVarUnit+ " addItemToVest '%1'; ", _x];
            };
    } foreach ((_array) select 0);
};
...
if (count(_array)>0) then {
    {
        for [{_i=0},{_i<((_array) select 1) select _forEachIndex},{_i=_i+1}] do {
                _gearText = _gearText + format [_nameVarUnit+ " addItemToBackpack '%1'; ", _x]; ///////WEIRD?????\\\\\\\\\\
            };
    } foreach ((_array) select 0);
};
...
if (count(_array)>0) then {
    {
        for [{_i=0},{_i<((_array) select 1) select _forEachIndex},{_i=_i+1}] do {
                _gearText = _gearText + format [_nameVarUnit+ " addItemToBackpack '%1'; ", _x];
            };
    } foreach ((_array) select 0);
};
...
{
    _gearText = _gearText + format [_nameVarUnit+ " linkItem '%1'; ", _x];
}
...
_strList= _gearText splitString ";";
{
...
hint "Loadout sent to server";

// Send the loadout to the server:

STW_PS_LOADOUT=[[(name _unit),_gearText]];

publicVariableServer "STW_PS_LOADOUT";
