...
if !( ( _this select [ 0, 1 ] ) params [
	...
	if ( _menu isEqualTo [] || ( _menu isEqualType "" && { isNil{ missionNamespace getVariable [ _menu, nil ] } } ) ) then {
...
if ( _shared isEqualTypeAny [ objNull, sideUnknown, grpNull, [], 0 ] || ( _shared isEqualType true && { _shared } ) ) then {
	if ( _shared isEqualType true ) then {
		_shared = [ 0, -2 ] select isDedicated;
	};
	private _JIP_ID = [ _menu, _vehicle, false, _menuDistance, _controls ] remoteExec[ "LARs_fnc_menuStart", _shared, true ];
	_vehicle setVariable[ "LARs_actionMenuJIPID", _JIP_ID ];
...
private _defaultActions = [];
{
	if ( _controls select _forEachIndex ) then {
		private _nul = _defaultActions pushBack [ _x, ( _vehicle addAction _x ) ];
	};
}forEach [
	...
	[ format[ "<t color='%1'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_sidebar_hide_up.paa' size='0.7' /> MENU HOME</t>", ACTIONMENUCOLOR ], { ( _this select 0 ) call LARs_fnc_menuReset }, "", -1, false, false, "", format [ "count ( _target getvariable [ 'LARs_menuDepth', [] ] ) > 1 && _this distance _target < %1", _menuDistance ] ],
	//MENU BACK - Only show if we are deeper than base menu
	[ format[ "<t color='%1'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_sidebar_show.paa' size='0.7' /> MENU BACK</t>", ACTIONMENUCOLOR ], { ( _this select 0 ) call LARs_fnc_menuDOWN }, "", -20, false, false, "", format [ "count ( _target getvariable [ 'LARs_menuDepth', [] ] ) > 0 && _this distance _target < %1", _menuDistance ] ],
	//MENU EXIT - Remove menu and clear variables
	[ format[ "<t color='%1'><img image='\A3\ui_f\data\gui\rsc\rscdisplayarcademap\icon_exit_cross_ca.paa' size='0.7' /> REMOVE MENU</t>", ACTIONMENUCOLOR ], { ( _this select 0 ) call LARs_fnc_menuExit }, "", -20, false, false, "", format [ "_this distance _target < %1", _menuDistance ] ]
];
		...
		while { true } do {
			...
			if ( !isNull cursorTarget ) then {
				if ( cursorTarget getVariable [ "LARs_menuSystemActive", false ] && { player distance cursorTarget <= ( cursorTarget getVariable [ "LARs_MenuDistance", 15 ] ) } ) then {
					...
					private _defaultActions = [];
					{
						private _nul = _defaultActions pushBack [ _x select 0, player addAction ( _x select 0 ) ];
					}forEach ( player getVariable [ "LARs_defaultActions", [] ] );
